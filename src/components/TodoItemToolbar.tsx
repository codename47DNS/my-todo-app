...
function TodoItemToolbar({ todo, updateId }: TodoItemToolbarType) {
  ...
  return (
    <div className="flex pb-1 ms-auto">
      <IconButton className="px-1 relative overflow-hidden" onClick={() => { copy(todo.title); }}>
        ...
        <CopyIcon className={`z-10 sm:opacity-0 ${copyStatus ? "" : "group-hover:opacity-100"} duration-300 text-blue-400`}></CopyIcon>
      </IconButton>
      ...
      {!todo.active && (
        ...
        >
          {todo.id === updateId ? (
            ...
            <UpdateIcon className="text-rose-400 sm:opacity-0 group-hover:opacity-100 duration-100" />
          )}
        </IconButton>
      )}
      ...
      <IconButton className="px-1 relative">
        ...
        <BookmarkIcon className="stroke-[--flag-color] stroke-[70px] text-transparent duration-100 sm:opacity-0 peer-checked/bookmark:opacity-100 group-hover:opacity-100"></BookmarkIcon>
      </IconButton>
      ...
      <IconButton className="px-1 relative">
        ...
        <StarIcon className="dark:stroke-orange-300 stroke-orange-400 peer-checked/star:text-orange-400 stroke-[70px] text-transparent peer-checked/star:dark:text-orange-300 duration-100 sm:opacity-0 peer-checked/star:opacity-100 group-hover:opacity-100"></StarIcon>
      </IconButton>

      {todo.id !== updateId && (
        <IconButton
          className="px-1 text-red-400 duration-100 sm:opacity-0 group-hover:opacity-100"
